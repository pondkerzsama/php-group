        ...
        <div class="bg-white dark:bg-gray-800 rounded-xl shadow-md overflow-hidden">
                            ...
                            <td class="p-4 text-center">
                                <form action="actions/update_cart.php" method="POST" class="inline-flex items-center gap-2 border dark:border-gray-600 rounded-lg px-2 py-1">
                                    ...
                                    <button type="submit" name="action" value="decrease" 
                                        class="font-bold px-2 <?= $item['quantity'] <= 1 ? 'text-gray-300 cursor-not-allowed opacity-50' : 'text-gray-500 dark:text-gray-400 hover:text-indigo-600' ?>"
                                        <?= $item['quantity'] <= 1 ? 'disabled' : '' ?>>-</button>
                                    <span class="w-8 text-center font-semibold"><?= $item['quantity'] ?></span>
                                    <button type="submit" name="action" value="increase" 
                                        class="font-bold px-2 <?= $item['quantity'] >= $item['stock'] ? 'text-gray-300 cursor-not-allowed opacity-50' : 'text-gray-500 dark:text-gray-400 hover:text-indigo-600' ?>"
            ...
            <div class="p-6 bg-gray-50 dark:bg-gray-800 border-t dark:border-gray-700 flex flex-col sm:flex-row justify-between items-center gap-4">
                ...
                <div class="flex items-center gap-6">
                    ...
                    <a href="checkout.php" class="bg-indigo-600 hover:bg-indigo-700 text-white px-8 py-3 rounded-xl font-bold transition shadow-lg whitespace-nowrap">ดำเนินการชำระเงิน →</a>
                </div>
            </div>
        </div>
    <?php else: ?>
